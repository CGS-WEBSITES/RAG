...
pool = AsyncConnectionPool(
    DB_URL, min_size=5, max_size=10, open=False, configure=setup_pgvector_psycopg
)
    ...
    async with conn.cursor() as cur:
        await cur.execute(
            ...
            SELECT ai.create_vectorizer(
                ...
                destination => ai.destination_table('wiki_embedding')
            )
            """
        )
    await conn.commit()
...
async def _find_relevant_chunks(client, query: str, limit: int = 1) -> List[WikiSearchResult]:
    """
    Find the most relevant text chunks for a given query using vector similarity search.
    Uses local Ollama embeddings (OpenAI-compatible /v1).
    """
    # Embedding local via Ollama (modelo de embeddings, não o llama3)
...
async def run():
    ...
    results = await _find_relevant_chunks(
        client, "Who is the father of computer science?"
    )
    ...
    query = "Who is the father of computer science?"
